-- Fix duplicate groups by handling existing groups properly
...
-- Delete groups with null organization_id that would create duplicates
-- (i.e., groups whose names already exist with the target organization_id)
...
-- Now delete remaining duplicates with null organization_id, keeping only the first one of each name
DELETE FROM public.groups 
WHERE id IN (
  SELECT id FROM (
    SELECT id, 
