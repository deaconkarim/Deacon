...
export const addEvent = async (event) => {
  try {
    ...
    if (event.is_recurring) {
      ...
      // Create reminder configurations for recurring events if enabled
...
export const updateEvent = async (id, updates) => {
  try {
    ...
    // CRITICAL FIX: If event not found by ID, it's likely a synthetic ID from the frontend
    if (fetchError && fetchError.code === 'PGRST116') {
      console.log('Event ID not found in database:', id);
      console.log('This is likely a synthetic ID generated by the frontend');
      
      // For synthetic IDs, we need to find the actual master event
      // Try to find by title first (most reliable for recurring events)
      if (updates.title) {
        console.log('Searching for master event by title:', updates.title);
        ...
        if (eventsByTitle && eventsByTitle.length > 0) {
          ...
          console.log('✅ Found master event by title:', originalEvent.id);
          
          // Mark this as a virtual instance edit if the original ID was synthetic
          if (id.includes('_') || id.includes('-2025-')) {
            console.log('🔄 Marking as virtual instance edit');
            originalEvent.is_virtual_instance_edit = true;
            originalEvent.requested_start_date = updates.startDate;
            originalEvent.requested_end_date = updates.endDate;
          }
        } else {
          throw new Error(`No master event found with title: ${updates.title}`);
        }
      } else {
        throw new Error(`Event not found with ID: ${id} and no title provided for fallback search`);
      }
    } else if (fetchError) {
      throw fetchError;
    }
    ...
    // If it's a recurring event, handle updates differently
    if (originalEvent.is_recurring) {
      ...
      if (isEditingInstance || isDefinitelyInstance || hasTimestampPattern || originalEvent.is_virtual_instance_edit) {
        ...
        const instanceData = {
          title: updates.title,
          description: updates.description,
          start_date: originalEvent.requested_start_date || updates.startDate,
          end_date: originalEvent.requested_end_date || updates.endDate,
          location: updates.location,
          location_id: updates.location_id || null,
          url: updates.url,
          event_type: updates.event_type || 'Worship Service',
        ...
        const searchStartDate = originalEvent.requested_start_date || updates.startDate;
        const { data: existingInstance, error: existingError } = await supabase
          .from('events')
          .select('*')
          .eq('parent_event_id', masterId)
          .eq('start_date', searchStartDate)
          .eq('organization_id', organizationId)
          .maybeSingle();
        ...
        if (instances.length > 0) {
          ...
          if (instancesError) {
            console.error('Error regenerating instances:', instancesError);
          } else {
            console.log(`Regenerated ${instances.length} instances for master event ${masterId}`);
          }
        }
