...
export function Dashboard() {
  ...
  const donationTrend = dashboardCalculations.getTrendIndicator(
    ...
    stats.donations.lastWeek || stats.donations.weekly
  );
  ...
  const generateSmartInsights = () => {
    ...
    if (stats.donations.thisWeek !== stats.donations.lastWeek) {
      const change = stats.donations.thisWeek - (stats.donations.lastWeek || stats.donations.weekly);
      const isPositive = change > 0;
      insights.push({
        title: isPositive ? 'Financial Growth' : 'Financial Focus Needed',
        category: 'Financial Health',
        description: `Donations are ${isPositive ? 'trending upward' : 'declining'} this week. ${isPositive ? 'Keep communicating impact.' : 'Consider stewardship emphasis.'}`,
        ...
        metrics: [
          ...
          { label: 'Monthly', value: dashboardCalculations.formatCurrency(stats.donations.monthly) }
        ]
      });
    }
  ...
  return (
    ...
    >
      <div className="w-full max-w-7xl mx-auto min-w-0 px-2 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8 overflow-x-hidden">
        ...
        {/* Optimized Header with Request Counter */}
        <motion.div className="mb-4 sm:mb-8 relative" variants={itemVariants}>
          <div className="absolute inset-0 bg-gradient-to-r from-emerald-600/5 to-green-600/5 blur-3xl rounded-3xl"></div>
          <div className="relative backdrop-blur-sm bg-white/90 dark:bg-slate-900/95 border border-white/30 dark:border-slate-700/50 rounded-2xl sm:rounded-3xl p-3 sm:p-6 shadow-xl">
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between space-y-6 lg:space-y-0">
              <div className="flex-1">
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-emerald-600 via-green-600 to-emerald-700 bg-clip-text text-transparent mb-2">
                  ⚡ Optimized Dashboard
                </h1>
                <p className="text-slate-600 dark:text-slate-300 text-lg font-medium mb-4">
                  Ultra-fast Church Management with Minimal Requests
                </p>
                <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                  <div className="flex items-center space-x-2">
                    <div className="w-3 h-3 bg-emerald-500 rounded-full animate-pulse shadow-lg shadow-emerald-500/50"></div>
                    <span className="text-sm text-emerald-600 dark:text-emerald-400 font-medium">Only 6-7 Requests Total</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="w-3 h-3 bg-blue-500 rounded-full animate-pulse shadow-lg shadow-blue-500/50"></div>
                    <span className="text-sm text-blue-600 dark:text-blue-400 font-medium">Smart Caching</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="w-3 h-3 bg-purple-500 rounded-full animate-pulse shadow-lg shadow-purple-500/50"></div>
                    <span className="text-sm text-purple-600 dark:text-purple-400 font-medium">Client-side Processing</span>
                  </div>
                </div>
                <div className="mt-4 px-4 py-2 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800">
                  <p className="text-sm text-emerald-700 dark:text-emerald-300 font-medium">
                    🚀 <strong>Performance Boost:</strong> Reduced from 531 requests to just 6-7 requests (98.7% reduction)
                  </p>
                </div>
              </div>
              ...
              <div className="flex-shrink-0">
                <button
                  onClick={refresh}
                  disabled={isLoading}
                  className="px-6 py-3 bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 text-white font-semibold rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 disabled:opacity-50"
                >
                  <RefreshCw className={`h-4 w-4 mr-2 inline ${isLoading ? 'animate-spin' : ''}`} />
                  {isLoading ? 'Loading...' : 'Refresh (6 requests)'}
                </button>
              </div>
            </div>
          </div>
              ...
              <motion.div variants={itemVariants} className="mb-6 sm:mb-12">
                <div className="text-center py-12">
                  <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">
                    Welcome to Your Optimized Dashboard
                  </h2>
        ...
        {/* Main Analytics Grid - All data from single optimized query */}
          ...
          <PermissionFeature permission={PERMISSIONS.DONATIONS_VIEW}>
            <StatCard
              ...
              metrics={[
                ...
                { label: 'Total', value: dashboardCalculations.formatCurrency(stats.donations.total) }
              ]}
        ...
        {/* Smart Insights - Generated client-side */}
        ...
        {/* Attendance Analytics - Calculated client-side from event data */}
        <PermissionFeature permission={PERMISSIONS.REPORTS_VIEW}>
          <motion.div variants={itemVariants} className="mb-6 sm:mb-12">
              ...
              <div className="relative backdrop-blur-sm bg-white/80 dark:bg-slate-800/80 border border-white/20 dark:border-slate-700/20 rounded-3xl p-3 sm:p-6 lg:p-8 shadow-xl hover:shadow-2xl transition-all duration-300">
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 dark:text-slate-400">Service participation insights (calculated client-side)</p>
                  </div>
                </div>
                ...
                <div className="grid gap-4 grid-cols-1 sm:grid-cols-3">
                  ...
                  <div className="text-center p-4 bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-950 dark:to-emerald-900 rounded-xl border border-emerald-200 dark:border-emerald-800">
                    ...
                    <div className="text-2xl font-bold text-emerald-700 dark:text-emerald-300">
                      {Math.round(stats.attendance.bibleStudyAttendance / Math.max(stats.attendance.bibleStudyEvents, 1)) || 0}
                    </div>
                  ...
                  <div className="text-center p-4 bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-950 dark:to-amber-900 rounded-xl border border-amber-200 dark:border-amber-800">
                    ...
                    <div className="text-2xl font-bold text-amber-700 dark:text-amber-300">
                      {Math.round(stats.attendance.fellowshipAttendance / Math.max(stats.attendance.fellowshipEvents, 1)) || 0}
                    </div>
        ...
        <motion.div variants={itemVariants} className="mb-6">
          <div className="text-center py-8 px-4 bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-900/20 dark:to-green-900/20 rounded-2xl border border-emerald-200 dark:border-emerald-800">
            <h4 className="text-lg font-bold text-emerald-900 dark:text-emerald-100 mb-2">
              🚀 Performance Optimized
            </h4>
            <p className="text-emerald-700 dark:text-emerald-300">
              This dashboard loads with only <strong>6-7 database requests</strong> instead of 531+ requests.
              All calculations are performed client-side for lightning-fast performance.
            </p>
          </div>
        </motion.div>
