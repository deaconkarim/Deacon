...
export function Permissions() {
  ...
  const handleBulkRoleUpdate = async () => {
    try {
      ...
      toast({
        ...
        description: `${selectedUsers.length} users have been updated to ${bulkRole} role`,
      });
    } catch (error) {
  ...
  const handleCreateRole = async () => {
    try {
      ...
      // For now, we'll store custom roles in localStorage
      ...
      localStorage.setItem(`custom_roles_${organizationId}`, JSON.stringify(updatedRoles));
  ...
  useEffect(() => {
    if (organizationId) {
      const savedRoles = localStorage.getItem(`custom_roles_${organizationId}`);
      if (savedRoles) {
        try {
        ...
        <Tabs defaultValue="users" className="space-y-4">
          ...
          <TabsContent value="roles" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {Object.entries(DEFAULT_ROLES).map(([roleKey, roleValue]) => (
                <Card key={roleValue}>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {roleValue === 'member' && 'Basic access to view information and participate in events'}
                      {roleValue === 'staff' && 'Can create and manage events, tasks, and basic operations'}
                      {roleValue === 'admin' && 'Full access to all features including user management'}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        {(DEFAULT_ROLE_PERMISSIONS[roleValue] || []).map((permission) => {
                      ...
                      {Object.entries(permissionCategories).map(([category, perms]) => (
                        <React.Fragment key={category}>
                          <tr className="bg-muted/50">
                            <td className="p-2 font-medium" colSpan={4}>{category}</td>
                          </tr>
